    ...
    fallback() external payable {
        assembly {
            ...
            function ensurePendingOwner() {
                ...
                if iszero(eq(caller(), pendingOwner)) { revert(0, 0) }
            }
            ...
            function ensureTokenOwner(tokenId) {
                ...
                if iszero(or(eq(caller(), owner), eq(caller(), tokenOwner))) { revert(0, 0) }
            }

            switch shr(224, calldataload(0x00))
            case 0xd8f361ad {
                // mint(bytes32,bytes32,address)
                ensureOwner()
            ...
            case 0xbdce9bde {
                // updateApproval(bytes32,address)
                let tokenId := calldataload(0x04)
            ...
            case 0x169dbe24 {
                // updateMetadata(bytes32,address)
                let tokenId := calldataload(0x04)
