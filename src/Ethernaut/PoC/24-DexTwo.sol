...
contract DexTwoAttack is Test {
    function testDexTwo() external {
        ...
        emit log('=========================================');
        emit log('pada fungsi swap, tidak ada persyaratn token yang diswap harus token1 dan token2\nARtinya kita bisa menggunakan token buatan sendiri untuk diswap');
        ...
        emit log('ga bisa pake fungsi add_liquidity() karena trader bukan owner dex. Untuk mengakalinya bisa pake erc20.transfer');
        ...
        micin.approve(address(dextwo), type(uint).max);
