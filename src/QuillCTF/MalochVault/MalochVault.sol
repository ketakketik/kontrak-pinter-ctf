...
contract MOLOCH_VAULT {
    ...
    function initiation(address payable _id, string memory _passW) public returns (bool) {
        ...
        uint256 lengthBefore = updatedCabal.length;
        Cabal memory newCabal = Cabal({password: _passW, identity: _id});
        realCabals.push(newCabal);
        uint256 lengthAfter = realCabals.length;
        bool initiated = true;
    ...
    function uhER778(string[3] memory _openSecrete) public payable {
        uint256 RGDTjhU = address(this).balance;
        ...
        require(success);
        uint256 YHUiiFD = address(this).balance;
        require(YHUiiFD - RGDTjhU == 1, "sacrifice your shallow desires");
        realHacker[msg.sender] = true;
    }

    constructor(string memory molochPass, string[2] memory _b, address payable[3] memory a, string[3] memory _passss)
        payable
    {
        /*
        ...
        */
        ...
        for (uint256 i = 0; i < a.length; i++) {
            Cabal memory aMember = Cabal({password: _passss[i], identity: a[i]});
            cabals.push(aMember);
        }
    }
}
